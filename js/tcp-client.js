...
Orignal Author: Boris Smus (smus@chromium.org)

Modified by: Benjamin Chase* (benjamin.chase@fh-luebeck.de)
* changed sending and logging settings.
...
TcpClient.prototype.sendMessage = function(msg, callback) {
	...
	//send raw messages from main script
...
TcpClient.prototype._onReceiveError = function(info) {
	if (info.socketId != this.socketId)
		return;
	// A negative ResultCode is an unrecoverable failure.  Close the Socket.
	if (info.resultCode < 0) {
		// log('connection closed by peer' + JSON.stringify(info));
		this.disconnect(this.socketId);
	}
	// error('Unable to receive data from socket: ' + JSON.stringify(info));
};
